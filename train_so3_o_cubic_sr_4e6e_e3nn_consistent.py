#!/usr/bin/env python3
"""
Train SO(3)/O cubic-quotient SR model (e3nn-consistent 4e/6e) on quaternion SR datasets.

This script is config-driven and uses:
  - model/loss from `so3_o_cubic_sr_4e6e_e3nn_consistent.py`
...
class TrainConfig:
    ...
    lam_contrast: float = 0.6
    lam_cross_aff: float = 1.0
    lam_interior: float = 0.08
    affinity_target_tau: float = 0.35
    ...
    hard_mask_mode: str = "hard"      # none, hard, soft
...
def load_train_config(config_path: Path, exp_dir: Path) -> TrainConfig:
    ...
    if not cfg.dataset_root:
        raise ValueError("Config must set `dataset_root` for dataset-backed training.")

    if not cfg.checkpoint_dir:
        cfg.checkpoint_dir = str(exp_dir / "checkpoints_so3o_sr_4e6e_e3nn_consistent")
...
def _extract_lr_hr_from_batch(batch: Any) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if isinstance(batch, dict):
        ...
        if lr is None or hr is None:
            raise KeyError("Batch dict must contain lr/hr tensors under keys `lr_q`/`hr_q` or `lr`/`hr`.")
        return lr, hr
...
def move_batch_to_device(batch: Any, device: torch.device, dtype: torch.dtype) -> Tuple[torch.Tensor, torch.Tensor]:
    lr_q, hr_q = _extract_lr_hr_from_batch(batch)

    lr_q = _to_channel_first_quat_map(lr_q).to(device=device, dtype=dtype, non_blocking=True)
    hr_q = _to_channel_first_quat_map(hr_q).to(device=device, dtype=dtype, non_blocking=True)
...
def load_checkpoint(
    ...
    incompatible = model.load_state_dict(ckpt["model_state_dict"], strict=strict_model_load)
...
def _compute_step_losses(
    ...
    tau_tgt = cfg.affinity_target_tau if cfg.affinity_target_tau > 0 else cfg.affinity_tau
    with torch.no_grad():
        ...
        boundary_gt = affinity_to_boundary(aff_gt, center_idx=center_idx).clamp(0.0, 1.0)

    losses = combined_e3nn_consistent_loss(
        model=model,
        pred=pred,
        hr_q_gt=hr_q_gt,
        gt_boundary_hr=boundary_gt,
        lam_shell=cfg.lam_shell,
        lam_boundary=cfg.lam_boundary,
        lam_dice=cfg.lam_dice,
        lam_affinity=cfg.lam_affinity,
        lam_contrast=cfg.lam_contrast,
        lam_cross_aff=cfg.lam_cross_aff,
        lam_interior=cfg.lam_interior,
    )
    ...
    if "boundary" in losses:
        out["loss_boundary_hr"] = losses["boundary"]
    if "affinity" in losses:
        out["loss_affinity_hr"] = losses["affinity"]
    if "contrast" in losses:
        out["loss_affinity_couple"] = losses["contrast"]
    if "cross_aff" in losses:
        out["loss_cross_aff"] = losses["cross_aff"]
    if "interior" in losses:
        out["loss_interior"] = losses["interior"]
...
def train_one_epoch(
    ...
    for batch in iterator:
        ...
        loss_terms = _compute_step_losses(model=model, pred=pred, hr_q_gt=hr_q, cfg=cfg)
        ...
        if "loss_boundary_hr" in loss_terms:
            total_bdry += float(loss_terms["loss_boundary_hr"].item())
            total_aff += float(loss_terms["loss_affinity_hr"].item())
            total_couple += float(loss_terms["loss_affinity_couple"].item())
            ...
            if "loss_cross_aff" in loss_terms:
                total_cross_aff += float(loss_terms["loss_cross_aff"].item())
            if "loss_interior" in loss_terms:
                total_interior += float(loss_terms["loss_interior"].item())
            n_aux_batches += 1
        n_batches += 1
    ...
    if n_aux_batches > 0:
        ...
        out["cross_aff"] = total_cross_aff / aux_denom
        out["interior"] = total_interior / aux_denom
    return out
...
def evaluate(
    ...
    for batch in iterator:
        ...
        if remaining_decode > 0 and q_dict is not None:
            ...
            x_pred_small = x_pred[:b].permute(0, 2, 3, 1).contiguous()
...
def _resolve_resume_path(checkpoint_dir: Path, resume: bool, resume_from: str = "") -> Optional[Path]:
    if resume_from:
        p = Path(resume_from)
...
def train_from_dataset(
    ...
    variant = str(cfg.model_variant).strip().lower()
    model_cls = SO3OQuotientSRNet4e6eE3NNLite if variant in ("lite", "light", "lightweight") else SO3OQuotientSRNet4e6eE3NN
    ...
    model = model_cls(
        ...
        affinity_tau=cfg.affinity_tau,
        affinity_hidden=cfg.affinity_hidden,
        hard_mask_mode=cfg.hard_mask_mode,
        hard_mask_threshold=cfg.hard_mask_threshold,
        hard_mask_temperature=cfg.hard_mask_temperature,
        stage_refine_blocks=cfg.stage_refine_blocks,
        dtype=dtype,
    ...
    print(f"Dtype: {dtype}")
    print("Learnable module: so3_o_cubic_sr_4e6e_e3nn_consistent")
    ...
    if start_epoch > cfg.epochs:
        print(f"Nothing to train: start_epoch ({start_epoch}) > epochs ({cfg.epochs}).")
    ...
    for epoch in range(start_epoch, cfg.epochs + 1):
        ...
        if epoch % max(cfg.save_every, 1) == 0:
            save_checkpoint(
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train SO(3)/O cubic quotient SR model (e3nn-consistent 4e6e)")
    parser.add_argument("--exp_dir", type=str, required=True, help="Experiment directory containing config JSON.")
    parser.add_argument("--config", type=str, default="config.json", help="Config filename inside exp_dir (default: config.json).")
    parser.add_argument("--resume", action="store_true", help="Resume from last.pt (or best.pt if last.pt is missing).")
    parser.add_argument(
        "--resume_from",
        type=str,
        default="",
