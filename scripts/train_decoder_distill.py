...
def main() -> None:
    ...
    if "q_input" in blob:
        ...
        raise ValueError(
            "Teacher table is missing `q_input` (or legacy `q_input_bunge`). Re-export teacher table."
        )
    ...
    for epoch in range(args.epochs):
        ...
        print(
            ...
            f"val_mis_mean_deg={mis_mean:.4f} val_mis_max_deg={mis_max:.4f}"
        )
    ...
    if best_state is None:
        best_state = {"decoder_state_dict": model.state_dict(), "meta": {"teacher_table": str(args.teacher_table)}}
