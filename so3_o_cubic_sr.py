...
def descriptor_to_local_affinity(
    ...
    """
    Analytic local affinity from descriptor-space distances.

    x_desc: [B,C,H,W]
    returns: [B,K,H,W], K=kernel_size^2
    """
    ...
    nei = F.unfold(x_pad, kernel_size=kernel_size, padding=0)  # [B, C*K, H*W]
    nei = nei.view(B, C, k2, H, W)
    ctr = x_desc.unsqueeze(2)  # [B,C,1,H,W]

    d2 = ((nei - ctr) ** 2).sum(dim=1)  # [B,K,H,W]
    a = torch.exp(-d2 / (tau * tau))
    a = a.clamp(min=eps, max=1.0 - eps)
    return replace_affinity_center(a, center_idx=center_idx, value=1.0)


def replace_affinity_center(
    affinity: torch.Tensor,
    center_idx: int,
    value: float = 1.0,
) -> torch.Tensor:
    """
    Return a copy of affinity with the center channel replaced by a constant.

    This avoids in-place edits that can break autograd graphs.
    """
    B, K, H, W = affinity.shape
    if center_idx < 0 or center_idx >= K:
        raise ValueError(f"center_idx {center_idx} out of range for K={K}")
    center = torch.full(
        (B, 1, H, W),
        float(value),
    ...
    left = affinity[:, :center_idx]
    right = affinity[:, center_idx + 1:]
    return torch.cat([left, center, right], dim=1)


def affinity_to_boundary(affinity: torch.Tensor, center_idx: Optional[int] = None) -> torch.Tensor:
    """
    Convert per-offset affinity [B,K,H,W] into scalar boundary probability [B,1,H,W].

    boundary = 1 - mean(neighbor affinities excluding center)
    """
    B, K, H, W = affinity.shape
    if center_idx is None:
        center_idx = K // 2
    idx = [k for k in range(K) if k != center_idx]
    if not idx:
        return torch.zeros((B, 1, H, W), device=affinity.device, dtype=affinity.dtype)
    boundary = 1.0 - affinity[:, idx].mean(dim=1, keepdim=True)
    return boundary.clamp(0.0, 1.0)


def boundary_pair_max(boundary: torch.Tensor, kernel_size: int) -> torch.Tensor:
    """
    For each stencil offset, compute max(center_boundary, neighbor_boundary).
    ...
    if C != 1:
        raise ValueError(f"boundary must have shape [B,1,H,W], got {tuple(boundary.shape)}")

    p = kernel_size // 2
    k2 = kernel_size * kernel_size
    b_pad = F.pad(boundary, (p, p, p, p), mode="replicate")
    b_nei = F.unfold(b_pad, kernel_size=kernel_size, padding=0).view(B, k2, H, W)
    b_ctr = boundary.expand(-1, k2, -1, -1)
    return torch.maximum(b_ctr, b_nei)


# ============================================================
# 11) Affinity heads + masked upsampling
# ============================================================

class DescriptorAffinityHead(nn.Module):
    """
    Per-offset local affinity from descriptor geometry + learned residual logits.

    Affinity is predicted on the LR grid for a KxK stencil centered at each pixel.
    """
    ...
    def __init__(
        ...
        self.num_offsets = int(kernel_size * kernel_size)
        self.center_idx = self.num_offsets // 2
        self.tau = float(tau)
        self.eps = float(eps)

        self.residual = nn.Sequential(
            nn.Conv2d(self.descriptor_dim, hidden, kernel_size=1),
            nn.SiLU(),
            nn.Conv2d(hidden, self.num_offsets, kernel_size=1),
        )
        # Start from analytic prior by default.
        nn.init.zeros_(self.residual[-1].weight)
        nn.init.zeros_(self.residual[-1].bias)

    def _analytic_affinity(self, x_desc: torch.Tensor) -> torch.Tensor:
        """
        x_desc: [B,9,H,W] (approximately shell-normalized)
        returns: [B,K,H,W] with K=kernel_size^2
        """
        if x_desc.shape[1] != self.descriptor_dim:
            raise ValueError(
                f"Expected {self.descriptor_dim} descriptor channels, got {x_desc.shape[1]}"
            )
        return descriptor_to_local_affinity(
            x_desc=x_desc,
    ...
    def forward(self, x_desc: torch.Tensor) -> torch.Tensor:
        a_prior = self._analytic_affinity(x_desc)  # [B,K,H,W]
        prior_logit = torch.logit(a_prior, eps=self.eps)
        delta = self.residual(x_desc)
        a = torch.sigmoid(prior_logit + delta)
        return replace_affinity_center(a, center_idx=self.center_idx, value=1.0)


class HRBoundaryCoupledAffinityHead(nn.Module):
    """
    HR auxiliary head that predicts:
      - boundary logits/probability
      - local affinities coupled to boundary strength

    Affinity logit:
        logit(a_ij) = logit(a_prior_ij) + delta_ij - gamma * max(b_i, b_j)
    """
    ...
    def __init__(
        ...
        self.descriptor_dim = int(descriptor_dim)
        self.kernel_size = int(kernel_size)
        self.num_offsets = self.kernel_size * self.kernel_size
        self.center_idx = self.num_offsets // 2
        self.tau = float(tau)
        self.eps = float(eps)
        ...
        self.boundary_head = nn.Sequential(
            nn.Conv2d(self.in_channels, h, kernel_size=3, padding=1),
            nn.SiLU(),
            nn.Conv2d(h, 1, kernel_size=1),
        )
        self.affinity_delta_head = nn.Sequential(
            nn.Conv2d(self.in_channels, h, kernel_size=3, padding=1),
            nn.SiLU(),
            nn.Conv2d(h, self.num_offsets, kernel_size=1),
        )

        # Start close to analytic affinity prior + no boundary penalty.
        nn.init.zeros_(self.affinity_delta_head[-1].weight)
        nn.init.zeros_(self.affinity_delta_head[-1].bias)
        nn.init.zeros_(self.boundary_head[-1].weight)
        nn.init.zeros_(self.boundary_head[-1].bias)

        coupling_init = max(float(coupling_init), 1e-3)
        self.gamma_raw = nn.Parameter(torch.tensor(math.log(math.exp(coupling_init) - 1.0)))

    def forward(self, x_desc: torch.Tensor, x_aux: torch.Tensor) -> dict[str, torch.Tensor]:
        """
        x_desc: [B,9,H,W] predicted HR descriptor
        x_aux : [B,in_channels-9,H,W] scalar HR hidden features
        """
        if x_desc.shape[1] != self.descriptor_dim:
            raise ValueError(
                f"Expected descriptor_dim={self.descriptor_dim}, got {x_desc.shape[1]}"
            )
        x_in = torch.cat([x_desc, x_aux], dim=1)
        if x_in.shape[1] != self.in_channels:
            raise ValueError(f"Expected {self.in_channels} aux channels, got {x_in.shape[1]}")

        affinity_prior = descriptor_to_local_affinity(
            x_desc=x_desc,
            kernel_size=self.kernel_size,
            tau=self.tau,
            eps=self.eps,
        )
        prior_logit = torch.logit(affinity_prior, eps=self.eps)

        boundary_logit = self.boundary_head(x_in)
        boundary_prob = torch.sigmoid(boundary_logit)
        boundary_pair = boundary_pair_max(boundary_prob, kernel_size=self.kernel_size)

        delta = self.affinity_delta_head(x_in)
        gamma = F.softplus(self.gamma_raw)
        affinity = torch.sigmoid(prior_logit + delta - gamma * boundary_pair)
        affinity = replace_affinity_center(affinity, center_idx=self.center_idx, value=1.0)

        return {
            "affinity_hr": affinity,
            "affinity_prior_hr": affinity_prior,
            "boundary_logit_hr": boundary_logit,
            "boundary_hr": boundary_prob,
            "boundary_pair_hr": boundary_pair,
            "boundary_gamma": gamma,
        }


class MaskedLocalUpsample2d(nn.Module):
    """
    Affinity-weighted local interpolation from LR -> HR.

    For each HR location, combines KxK LR neighbors with:
      - geometric weight (distance to continuous LR coordinate)
      - learned affinity weight from LR center pixel.
    """
    ...
    def __init__(
        ...
        super().__init__()
        if not isinstance(scale_factor, int) or scale_factor < 2:
            raise ValueError(f"scale_factor must be integer >=2, got {scale_factor}")
        if kernel_size % 2 != 1 or kernel_size < 1:
            raise ValueError(f"kernel_size must be odd and >=1, got {kernel_size}")
        if sigma <= 0:
        ...
        p = kernel_size // 2
        self.offsets = [(dy, dx) for dy in range(-p, p + 1) for dx in range(-p, p + 1)]

    @staticmethod
    def _to_norm(coord: torch.Tensor, size: int) -> torch.Tensor:
        if size <= 1:
            return torch.zeros_like(coord)
        return 2.0 * coord / float(size - 1) - 1.0

    def forward(self, x: torch.Tensor, affinity: torch.Tensor) -> torch.Tensor:
        """
        x: [B,C,H,W]
        affinity: [B,K,H,W], K=kernel_size^2
        returns: [B,C,sH,sW]
        """
        B, C, H, W = x.shape
        B2, K, H2, W2 = affinity.shape
        assert B == B2 and H == H2 and W == W2
        assert K == self.num_offsets, f"Expected {self.num_offsets} affinity channels, got {K}"
        ...
        # HR pixel centers mapped to continuous LR coordinates.
        y_hr = torch.arange(sH, device=device, dtype=dtype)
        x_hr = torch.arange(sW, device=device, dtype=dtype)
        yy_hr, xx_hr = torch.meshgrid(y_hr, x_hr, indexing="ij")

        yy_lr = (yy_hr + 0.5) / float(s) - 0.5
        xx_lr = (xx_hr + 0.5) / float(s) - 0.5

        base_y = torch.floor(yy_lr).clamp(0.0, float(max(H - 1, 0)))
        base_x = torch.floor(xx_lr).clamp(0.0, float(max(W - 1, 0)))
        fy = yy_lr - base_y
        fx = xx_lr - base_x

        base_grid = torch.stack(
            [self._to_norm(base_x, W), self._to_norm(base_y, H)],
            dim=-1,
        )  # [sH,sW,2]
        base_grid_b = base_grid.unsqueeze(0).expand(B, -1, -1, -1)

        out = torch.zeros((B, C, sH, sW), device=device, dtype=dtype)
        den = torch.zeros((B, 1, sH, sW), device=device, dtype=dtype)

        for k_idx, (dy, dx) in enumerate(self.offsets):
            qy = (base_y + float(dy)).clamp(0.0, float(max(H - 1, 0)))
            qx = (base_x + float(dx)).clamp(0.0, float(max(W - 1, 0)))
            q_grid = torch.stack(
                [self._to_norm(qx, W), self._to_norm(qy, H)],
                dim=-1,
            )
            q_grid_b = q_grid.unsqueeze(0).expand(B, -1, -1, -1)

            # LR center->neighbor affinity for this offset, sampled at base pixel.
            a_k = F.grid_sample(
                affinity[:, k_idx:k_idx + 1],
                base_grid_b,
                mode="nearest",
                padding_mode="border",
                align_corners=True,
            )
            # Neighbor feature sample.
            x_k = F.grid_sample(
                x,
                q_grid_b,
                mode="bilinear",
            ...
            w_geo = torch.exp(
                -((fy - float(dy)) ** 2 + (fx - float(dx)) ** 2) / (2.0 * self.sigma2)
            ).unsqueeze(0).unsqueeze(0)
            w = a_k * w_geo
            out = out + w * x_k
            den = den + w

        return out / den.clamp_min(self.eps)


class DualFiberMaskedInterpolation(nn.Module):
    """
    Shared affinity-driven local interpolation for 4e and 6e hidden fibers.
    """
    ...
    def forward(
        ...
        return self.up(x4, affinity), self.up(x6, affinity)


# ============================================================
# 11) Dual-fiber upsamplers
# ============================================================
...
class SO3OQuotientSRNet4e6e(nn.Module):
    ...
    def __init__(
        ...
        upsample_mode: str = "bilinear",   # "bilinear", "nearest", "bicubic", "transpose", "masked"
        sr_scale: int = 2,
        refine_per_stage: int = 1,
        passive_input: bool = True,
        ...
        masked_interp_sigma: float = 0.75,
        hr_aux_enabled: bool = True,
        hr_aux_coupling_init: float = 1.0,
        dtype: torch.dtype = torch.float64,
    ):
        super().__init__()
        self.sr_scale = sr_scale
        ...
        self.affinity_head: Optional[DescriptorAffinityHead] = None
        self.masked_skip_up: Optional[MaskedLocalUpsample2d] = None
        self.hr_aux_head: Optional[HRBoundaryCoupledAffinityHead] = None
        ...
        elif upsample_mode == "masked":
            self.affinity_head = DescriptorAffinityHead(
                descriptor_dim=9,
                kernel_size=affinity_kernel_size,
                hidden=affinity_hidden,
                tau=affinity_tau,
            )
            self.up = DualFiberMaskedInterpolation(
                scale_factor=sr_scale,
                kernel_size=affinity_kernel_size,
                sigma=masked_interp_sigma,
            )
            self.masked_skip_up = MaskedLocalUpsample2d(
                scale_factor=sr_scale,
                kernel_size=affinity_kernel_size,
                sigma=masked_interp_sigma,
            )
            if hr_aux_enabled:
                aux_in_ch = 9 + hidden_mul4 + hidden_mul6
                self.hr_aux_head = HRBoundaryCoupledAffinityHead(
                    in_channels=aux_in_ch,
                    descriptor_dim=9,
                    kernel_size=affinity_kernel_size,
                    hidden=affinity_hidden,
                    tau=affinity_tau,
                    coupling_init=hr_aux_coupling_init,
                )
        else:
    ...
    def predict_affinity(self, q_lr: torch.Tensor) -> torch.Tensor:
        """
        Predict LR local affinities for masked upsampling mode.

        returns: [B, K, H, W] where K = affinity_kernel_size^2
        """
        if self.affinity_head is None:
            raise RuntimeError("Affinity head is only available when upsample_mode='masked'.")
        x0 = self.encode(q_lr)
        return self.affinity_head(x0)

    def _hidden_scalar_features(self, x4: torch.Tensor, x6: torch.Tensor) -> torch.Tensor:
        """
        Build scalar HR features from hidden 4e/6e copies.

        returns: [B, hidden_mul4 + hidden_mul6, H, W]
        """
        B, _, H, W = x4.shape
        x4v = x4.reshape(B, self.hidden_mul4, 9, H, W)
        x6v = x6.reshape(B, self.hidden_mul6, 13, H, W)
        n4 = torch.sqrt((x4v ** 2).sum(dim=2).clamp_min(1e-12))
        n6 = torch.sqrt((x6v ** 2).sum(dim=2).clamp_min(1e-12))
        return torch.cat([n4, n6], dim=1)
    ...
    def forward_descriptor(
        ...
        affinity_lr = None
        if self.upsample_mode == "masked":
            if self.affinity_head is None or self.masked_skip_up is None:
                raise RuntimeError("Masked upsample requested but affinity modules are missing.")
            affinity_lr = self.affinity_head(x0)                 # [B,K,H,W]
            x4, x6 = self.up(x4, x6, affinity_lr)
        else:
            x4, x6 = self.up(x4, x6)
        ...
        if self.upsample_mode == "masked":
            assert affinity_lr is not None and self.masked_skip_up is not None
            skip = self.masked_skip_up(x0, affinity_lr)         # [B,9,sH,sW]
        else:
        ...
        if self.hr_aux_head is not None:
            aux_features = self._hidden_scalar_features(x4, x6)
            aux = self.hr_aux_head(x_desc=y, x_aux=aux_features)
        return y, aux
