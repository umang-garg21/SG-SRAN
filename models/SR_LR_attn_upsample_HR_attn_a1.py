...
"""
SR_LR_attn_upsample_HR_attn_a1.py
====================================
Local-iso SR model with LR attention, equivariant upsampling, and HR block attention.

Architecture pipeline:
  1. Encode LR quaternions to A1 features
  2. (Optional) LR equivariant spatial convolutions
  3. (Optional) LR block-local equivariant self-attention
  4. Upsample to HR grid (EquivariantTransposeConv)
  5. (Optional) HR equivariant spatial convolution
  6. (Optional) HR block-local equivariant self-attention
  7. Decode HR features to quaternions using optimizing decoder

New parameters vs SR_double_conv_SRattn_a1:
  - num_lr_attn_blocks: number of LR attention blocks (applied before upsampling)
  - lr_attn_block_size: spatial block size for LR attention
  - use_hr_conv1: whether to apply the HR spatial conv after upsampling
...
class IsoEmbeddingSRAttn(nn.Module):
    ...
    - Crystal family is selected at top-level with `crystal`:
      - `fcc` (group O)
      - `hcp` (group D6)
    - Pipeline (all stages run in `irreps_a1`):
      1. Encode LR quaternions to A1 features
      2. (Optional) LR equivariant spatial convolutions (conv_lr1/2/3)
      3. (Optional) LR block-local equivariant self-attention (lr_attn_blocks)
      4. Upsample to HR grid (upsample_conv)
      5. (Optional) HR equivariant spatial convolution (conv_hr1)
      6. (Optional) HR block-local equivariant self-attention (attention_blocks)
      7. Decode HR features to quaternions using optimizing decoder

    New vs SR_double_conv_SRattn_a1:
      - num_lr_attn_blocks: number of LR attention blocks (0 = disabled)
      - lr_attn_block_size: spatial block size for LR attention
      - use_hr_conv1: whether to apply HR spatial conv after upsampling
    """
    ...
    def __init__(
        ...
        num_lr_attn_blocks: int = 0,
        lr_attn_block_size: int = 8,
        ...
        self.lr_attn_block_size = int(lr_attn_block_size)
        ...
        # LR attention block(s)
        self.lr_attn_blocks = nn.ModuleList(
            [
                AttentionBlock(
                    self.irreps_a1,
                    num_channels=int(hr_attn_num_channels),
                ...
                for _ in range(int(num_lr_attn_blocks))
            ]
        )

        self._cached_hr_block_shape: tuple[int, int] | None = None
        self._cached_hr_sh_block: torch.Tensor | None = None
        self._cached_lr_block_shape: tuple[int, int] | None = None
        self._cached_lr_sh_block: torch.Tensor | None = None
    ...
    def _get_hr_sh_block(
        ...
        Used for position bias in HR attention blocks.
        """
        if (
    ...
    def _get_lr_sh_block(
        ...
        """
        Compute and cache the pairwise L2 distance matrix for a spatial block.
        Used for position bias in LR attention blocks. Separate cache from HR.
        """
        if (
            self._cached_lr_block_shape == (block_h, block_w)
            and self._cached_lr_sh_block is not None
            and self._cached_lr_sh_block.device == device
        ):
            return self._cached_lr_sh_block.to(dtype)
    ...
    def _apply_lr_attention(
        self,
        features: torch.Tensor,
        lr_shape: tuple[int, int],
    ) -> torch.Tensor:
        """
        Apply block-local equivariant attention to LR features (before upsampling).
        Handles padding so that all blocks are full-sized.
        """
        if len(self.lr_attn_blocks) == 0:
            return features
        ...
        B, N, C = features.shape
        if N != Hl * Wl:
            raise ValueError(f"Expected N={Hl*Wl} from lr_shape, got {N}")

        block_h = min(self.lr_attn_block_size, Hl)
        block_w = min(self.lr_attn_block_size, Wl)
        pad_h = (-Hl) % block_h
        pad_w = (-Wl) % block_w
        Hl_pad, Wl_pad = Hl + pad_h, Wl + pad_w

        feat = features
        if pad_h > 0 or pad_w > 0:
            feat_2d = feat.reshape(B, Hl, Wl, C).permute(0, 3, 1, 2)
            feat_2d = F.pad(feat_2d, (0, pad_w, 0, pad_h), mode="reflect")
            feat = feat_2d.permute(0, 2, 3, 1).reshape(B, Hl_pad * Wl_pad, C)

        sh_block = self._get_lr_sh_block(block_h, block_w, feat.device, feat.dtype)
        for block in self.lr_attn_blocks:
            delta = block(feat, sh_block, Hl_pad, Wl_pad, block_h, block_w)
            feat = feat + delta

        if pad_h > 0 or pad_w > 0:
            feat = feat.reshape(B, Hl_pad, Wl_pad, C)[:, :Hl, :Wl, :].reshape(B, Hl * Wl, C)
    ...
    def _forward_sr_features(
        ...
        """
        Main SR feature pipeline: optional LR convs, LR attention, upsampling,
        optional HR conv, HR attention.
        ...
        # LR attention (applied before upsampling for global LR context)
        feat = self._apply_lr_attention(feat, lr_shape)
