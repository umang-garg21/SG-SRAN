...
class EBSD_Ti64DIC_dataset(data.Dataset):
    ...
    def __init__(self, args, root_lr, root_hr, upsample_2d=True, is_Train=True):
        ...
        self.data_lr_files = (
            sorted(glob.glob(f"{root_lr}/*.npy")) if root_lr is not None else None
        )
    ...
    def __getitem__(self, index):
        ...
        filepath_lr = (
            self.data_lr_files[index] if self.data_lr_files is not None else None
        )
...
class Misorientation_dist:
    def __init__(self, args, dist_type="rot_dist", act=None, syms_req=True):
        ...
        print("+++++++++++++++++++++++++++++++++++++++++")
        print(
            f"dist_type: {dist_type}  activation:{act}  symmetry type:{syms_type} Symmetry:{syms_req}"
        )
        print("+++++++++++++++++++++++++++++++++++++++++++++++++")
        ...
        self.act_loss = ActAndLoss(
            act, Loss(dist_func=dist_type, syms=syms), quat_dim=-1
        )
...
class checkpoint:
    def __init__(self, args):
        ...
        if args.load == ".":
            if args.save == ".":
                args.save = now
            self.dir = (
                "/data/dkjangid/Material_Projects/superresolution/Quaternion_experiments/NMI_model_weights/"
                + args.save
            )
        else:
            self.dir = (
    ...
    def plot_val_loss(self, total_val_loss_all, epoch_list):
        ...
        plt.plot(axis, total_val_loss_all, label=label)
        plt.legend()
        plt.xlabel("Epochs")
    ...
    def save_results(self, filenames, save_list, postfix, scale, epoch, dataset="val"):
        ...
        for idx, file_name in enumerate(filenames):
            ...
            filename = "{}/results/{}_{}/{}_x{}".format(
                self.dir, dataset, epoch, file_name, scale
            )
            ...
            if self.args.n_channels != 3:
                for ch_num, channel in enumerate(channels):
                    ...
                    fig.suptitle(
                        f"{dataset} data: Filename:{file_name}_{channel}",
                        fontweight="bold",
                    )
...
def make_warmup_scheduler(args, my_optimizer):
    ...
    scheduler_cosine = optim.lr_scheduler.CosineAnnealingLR(
        my_optimizer, args.epochs - warmup_epochs, eta_min=args.optim_lr_min
    )
