...
class MaskedLearnableSharedOverMUpsample2d(nn.Module):
    """
    Learnable affinity-aware upsampler tied across m-components of one irrep block.

    For each LR pixel and each subpixel phase, a learned kernel mixes a local KxK
    LR neighborhood, but every offset is modulated by the predicted affinity mask.
    This gives a trainable upsampler without blind cross-grain averaging.
    """
    ...
    def __init__(
        ...
        if bias:
            self.bias = nn.Parameter(torch.zeros(self.num_phases, mul_out))
        else:
            self.bias = None
    ...
    def forward(self, x: torch.Tensor, aff: torch.Tensor) -> torch.Tensor:
        ...
        y = F.pixel_shuffle(y, upscale_factor=s)
        return y


class DualFiberMaskedLearnableUpsample(nn.Module):
    def __init__(
        self,
        mul4: int,
        mul6: int,
        ...
        super().__init__()
        self.up4 = MaskedLearnableSharedOverMUpsample2d(
            mul_in=mul4, mul_out=mul4, l=4, scale_factor=scale_factor, kernel_size=kernel_size, bias=bias, eps=eps
        )
        self.up6 = MaskedLearnableSharedOverMUpsample2d(
            mul_in=mul6, mul_out=mul6, l=6, scale_factor=scale_factor, kernel_size=kernel_size, bias=bias, eps=eps
        )

    def forward(self, x4: torch.Tensor, x6: torch.Tensor, aff: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        return self.up4(x4, aff), self.up6(x6, aff)
...
# ============================================================
# 8) 4e/6e lifting, fusion, and masked residual blocks
# ============================================================
...
class DescriptorHead46(nn.Module):
    def __init__(self, mul4: int, mul6: int):
        ...
        self.mul4 = mul4
        self.mul6 = mul6
        self.fuse6to4 = PerPixelLinearIrrep(f"{mul6}x6e", f"{mul4}x4e")
        self.mix4 = SharedOverMConv2d(mul4, mul4, l=4, kernel_size=3, padding=1)
        self.out4 = SharedOverMConv2d(mul4, 1, l=4, kernel_size=3, padding=1)

    def fused_4e(self, x4: torch.Tensor, x6: torch.Tensor) -> torch.Tensor:
        return self.mix4(x4 + self.fuse6to4(x6))

    def forward(self, x4: torch.Tensor, x6: torch.Tensor) -> torch.Tensor:
        return self.out4(self.fused_4e(x4, x6))


# ============================================================
# 9) Better boundary-aware 4e + 6e SR model
# ============================================================

class SO3OQuotientSRNet4e6eLearnableUpsample(nn.Module):
    def __init__(
        self,
        hidden_mul4: int = 16,
        hidden_mul6: int = 8,
        ...
        if str(upsample_mode).lower() != "masked_learnable":
            raise ValueError("This learnable version currently supports upsample_mode='masked_learnable' only.")
        ...
        self.lr_blocks = MaskedResidual46Stack(hidden_mul4, hidden_mul6, num_blocks=num_blocks_lr, kernel_size=affinity_kernel_size)

        self.up_hidden = DualFiberMaskedLearnableUpsample(
            hidden_mul4,
            hidden_mul6,
            scale_factor=sr_scale,
            kernel_size=affinity_kernel_size,
        )
        self.up_desc = MaskedLearnableSharedOverMUpsample2d(
            mul_in=1,
            mul_out=1,
            l=4,
            scale_factor=sr_scale,
        ...
        self.skip_up = MaskedLearnableSharedOverMUpsample2d(
            mul_in=1,
            mul_out=1,
            l=4,
            scale_factor=sr_scale,
        ...
        self.hr_blocks = MaskedResidual46Stack(hidden_mul4, hidden_mul6, num_blocks=num_blocks_hr, kernel_size=affinity_kernel_size)

        self.descriptor_head = DescriptorHead46(hidden_mul4, hidden_mul6)

    def encode(self, q_lr: torch.Tensor) -> torch.Tensor:
        return self.codec.encode_map(q_lr)

    def _normalize_descriptor(self, y_raw: torch.Tensor) -> torch.Tensor:
        target_norm = self.codec.a4.norm()
        ynorm = torch.sqrt((y_raw ** 2).sum(dim=1, keepdim=True).clamp_min(1e-12))
        return target_norm * y_raw / ynorm
    ...
    def forward(self, q_lr: torch.Tensor, lr_boundary_prior: Optional[torch.Tensor] = None) -> dict:
        ...
        x4, x6 = self.up_hidden(x4, x6, aff_lr)
        x_desc_u = self.up_desc(x0, aff_lr)
        skip = self.skip_up(x0, aff_lr)
        ...
        aff_hr0, boundary_hr0, boundary_hr0_logit = self.aff_hr_head(
            x4,
            x6,
            x_desc=x_desc_u,
        ...
        provisional_descriptor_hr_raw = self.descriptor_head(x4, x6) + skip
        provisional_descriptor_hr = self._normalize_descriptor(provisional_descriptor_hr_raw)

        aff_hr, boundary_hr, boundary_hr_logit = self.aff_hr_head(
            x4,
            x6,
            x_desc=provisional_descriptor_hr,
            boundary_prior=hr_boundary_prior,
        )

        x4, x6 = self.hr_blocks(x4, x6, aff_hr)

        y_raw = self.descriptor_head(x4, x6) + skip
        y = self._normalize_descriptor(y_raw)

        return {
            "descriptor_hr": y,
            "descriptor_hr_raw": y_raw,
            "provisional_descriptor_hr": provisional_descriptor_hr,
            "provisional_descriptor_hr_raw": provisional_descriptor_hr_raw,
            "affinity_lr": aff_lr,
            ...
            "boundary_hr0_logit": boundary_hr0_logit,
            "boundary_hr_logit": boundary_hr_logit,
            "boundary_lr_from_affinity": affinity_to_boundary(aff_lr, self.aff_lr_head.center_idx),
            "boundary_hr0_from_affinity": affinity_to_boundary(aff_hr0, self.aff_hr_head.center_idx),
            "boundary_hr_from_affinity": affinity_to_boundary(aff_hr, self.aff_hr_head.center_idx),
        }
...
SO3OQuotientSRNet4e6eBest = SO3OQuotientSRNet4e6eLearnableUpsample
SO3OQuotientSRNet4e6e = SO3OQuotientSRNet4e6eLearnableUpsample
...
def combined_boundary_aware_quotient_loss(
    model: SO3OQuotientSRNet4e6eLearnableUpsample,
    pred: dict,
    ...
    if gt_boundary_hr is not None:
        ...
        loss_aff0 = affinity_bce_from_boundary_loss(pred["affinity_hr0"], gt_boundary_hr, kernel_size=k)
        total = total + lam_boundary_bce * loss_b_bce + lam_boundary_dice * loss_b_dice + lam_affinity * loss_aff + lam_affinity0 * loss_aff0
        out.update({
...
def _demo():
    ...
    print("provisional_descriptor_hr:", tuple(out["provisional_descriptor_hr"].shape))
    print("affinity_lr  :", tuple(out["affinity_lr"].shape))
    print("affinity_hr0 :", tuple(out["affinity_hr0"].shape))
