...
class QFeedForward(nn.Module):
    def __init__(self, dim, ffn_expansion_factor):
        ...
        self.project_in = conv2d(
            dim, hidden_features * 2, kernel_size=1, stride=1, padding=0
        )
        ...
        self.project_out = conv2d(
            hidden_features, dim, kernel_size=1, stride=1, padding=0
        )
...
class First_Residual_D(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel=3, stride=1, spectral_normed=False
    ):
