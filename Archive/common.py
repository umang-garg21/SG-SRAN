...
def get_patch_1D(img_hr_in, patch_size, scale):
...
def get_patch_2D(img_hr_in, patch_size, scale):

    ih, iw = img_hr_in.shape[:2]

    iw_rand = random.randrange(0, iw - patch_size + 1)
    ih_rand = random.randrange(0, ih - patch_size + 1)

    # img_in = img_in[iy:iy + tp, ix:ix + tp, :]

    img_hr_in_patch = img_hr_in[
        ih_rand : ih_rand + patch_size,
        iw_rand : iw_rand + patch_size,
        :,
    ]

    # Downsample
    img_ds = img_hr_in_patch[:, ::scale, ::scale]

    # return img_ds, img_in
    return img_ds, img_hr_in_patch
