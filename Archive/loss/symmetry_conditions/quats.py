...
def hadamard_prod(q1, q2):
    assert _broadcastable(q1.shape, q2.shape), (
        "Inputs of shapes " f"{q1.shape}, {q2.shape} could not be broadcast together"
    )
    X1 = _vec2mat(q1.X)
...
class Quat:
    def __init__(self, X, use_torch=False, scalar_first=True, use_gpu=False):
        ...
        if not use_torch and not isinstance(X, torch.Tensor):
            ...
            self.X = torch.as_tensor(X).cuda() if use_gpu else torch.as_tensor(X).cuda()
        ...
        if not scalar_first:
            if _is_np(self.X):
                ...
                self.X = torch.roll(self.X, 1, dims=-1)
    ...
    def to_torch(self):
        return Quat(torch.as_tensor(self.X).float())
    ...
    def reshape(self, axes):
        if isinstance(axes, tuple):
            return Quat(self.X.reshape(axes + (4,)))
        else:
            return Quat(self.X.reshape((axes, 4)))
