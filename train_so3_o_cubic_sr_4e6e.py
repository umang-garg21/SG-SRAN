...
class TrainConfig:
    ...
    lam_boundary_hr: float = 0.0
    lam_affinity_hr: float = 0.0
    lam_affinity_couple: float = 0.0
    ...
    upsample_mode: str = "transpose"  # "transpose" or interpolation modes
    sr_scale: int = 4
...
def load_train_config(config_path: Path, exp_dir: Path) -> TrainConfig:
    ...
    if not cfg.checkpoint_dir:
        cfg.checkpoint_dir = str(exp_dir / "checkpoints_so3o_sr_4e6e")
...
def _compute_step_losses(
    ...
    x_gt: torch.Tensor,
    cfg: TrainConfig,
    aux_pred: Optional[Dict[str, torch.Tensor]] = None,
) -> Dict[str, torch.Tensor]:
    target_radius = model.codec.descriptor_radius
    mse_term = torch.mean((x_pred - x_gt) ** 2)
    shell_term = (
        (
            torch.sqrt((x_pred ** 2).sum(dim=1).clamp_min(1e-12)) - target_radius
        )
        ** 2
    ).mean()
    loss = quotient_descriptor_loss(
        x_pred=x_pred,
        x_gt=x_gt,
    ...
    out: Dict[str, torch.Tensor] = {
        "loss_total": loss,
        "loss_desc": mse_term,
        "loss_shell": shell_term,
    }

    use_aux = (
        aux_pred is not None
        and (
            cfg.lam_boundary_hr > 0
            or cfg.lam_affinity_hr > 0
            or cfg.lam_affinity_couple > 0
        )
    )
    if not use_aux:
        return out

    aff_pred = aux_pred.get("affinity_hr")
    b_logit = aux_pred.get("boundary_logit_hr")
    b_prob = aux_pred.get("boundary_hr")
    if aff_pred is None or b_logit is None or b_prob is None:
        return out

    k2 = int(aff_pred.shape[1])
    ...
    with torch.no_grad():
        ...
        b_gt = affinity_to_boundary(aff_gt, center_idx=center_idx)

    non_center_idx = [i for i in range(k2) if i != center_idx]
    if non_center_idx:
        aff_pred_nc = aff_pred[:, non_center_idx].clamp(1e-6, 1.0 - 1e-6)
        aff_gt_nc = aff_gt[:, non_center_idx].clamp(1e-6, 1.0 - 1e-6)
        loss_aff = F.binary_cross_entropy(aff_pred_nc, aff_gt_nc)
    else:
        loss_aff = torch.zeros((), device=x_pred.device, dtype=x_pred.dtype)

    loss_bdry = F.binary_cross_entropy_with_logits(b_logit, b_gt)
    b_from_aff = affinity_to_boundary(aff_pred, center_idx=center_idx)
    loss_couple = F.l1_loss(b_prob, b_from_aff)

    total = out["loss_total"]
    total = total + cfg.lam_boundary_hr * loss_bdry
    total = total + cfg.lam_affinity_hr * loss_aff
    total = total + cfg.lam_affinity_couple * loss_couple

    out["loss_total"] = total
    out["loss_boundary_hr"] = loss_bdry
    out["loss_affinity_hr"] = loss_aff
    out["loss_affinity_couple"] = loss_couple
    out["metric_boundary_hr_gt_mean"] = b_gt.mean()
    out["metric_boundary_hr_pred_mean"] = b_prob.mean()
    return out
...
def train_one_epoch(
    ...
    for batch in iterator:
        ...
        forward_out = model.forward_descriptor(lr_q, return_aux=True)
        if isinstance(forward_out, tuple):
            x_pred, aux_pred = forward_out
        else:
            x_pred = forward_out
            aux_pred = None
...
def train_from_dataset(
    ...
    model = SO3OQuotientSRNet4e6e(
        ...
        num_blocks_pre=cfg.num_blocks_pre,
        num_blocks_post=cfg.num_blocks_post,
        upsample_mode=cfg.upsample_mode,
        sr_scale=cfg.sr_scale,
        refine_per_stage=cfg.refine_per_stage,
        ...
        masked_interp_sigma=cfg.masked_interp_sigma,
        hr_aux_enabled=cfg.hr_aux_enabled,
        hr_aux_coupling_init=cfg.hr_aux_coupling_init,
        dtype=dtype,
